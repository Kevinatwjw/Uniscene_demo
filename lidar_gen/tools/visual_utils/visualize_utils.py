...
def visualize_pts(
    pts, fig=None, bgcolor=(0, 0, 0), fgcolor=(1.0, 1.0, 1.0), show_intensity=False, size=(600, 600), draw_origin=True
):
    ...
    if show_intensity:
        G = mlab.points3d(
            pts[:, 0], pts[:, 1], pts[:, 2], pts[:, 3], mode="point", colormap="gnuplot", scale_factor=1, figure=fig
        )
    else:
        G = mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], mode="point", colormap="gnuplot", scale_factor=1, figure=fig)
    if draw_origin:
