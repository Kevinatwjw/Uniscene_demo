...
class StandardDiffusionLoss(nn.Module):
    ...
    def forward(
        self, network: nn.Module, denoiser: Denoiser, conditioner: GeneralConditioner, input: torch.Tensor, batch: Dict
    ) -> torch.Tensor:
        cond = conditioner(batch)
        return self._forward(network, denoiser, cond, input)

    def _forward(self, network: nn.Module, denoiser: Denoiser, cond: Dict, input: torch.Tensor, noise_prior=True):
        ...
        if self.replace_cond_frames:
            ...
            for each_cond_mask in cond_mask:
                ...
                cond_indices = random.choices(self.cond_frames_choices, k=1)[0]
                if random.random() > 0.35:
                    cond_indices = random.sample(range(0, self.num_frames), random.randint(1, int(self.num_frames / 2)))
                else:
                    cond_indices = []

                if cond_indices:
                    if each_cond_mask.dim() == 2:  ## modify
                        each_cond_mask[:, cond_indices] = 1
                    else:
                        each_cond_mask[cond_indices] = 1
                    # each_cond_mask[cond_indices] = 1
            cond_mask = rearrange(cond_mask, "b v t -> (b v t)")  ## modify
        ...
        # if noise_prior: ## noise_prior
        #     noised_input = rearrange(noised_input, "(b v t) c h w-> b v t  c h w", t=self.num_frames, v=self.num_cameras, h=input.shape[-2], w=input.shape[-1] )
        #     for idx in range(1, self.num_frames):
        #         noised_input[:, :, idx, ...] =  noised_input[:, :, 0, ...]
        #     noised_input = rearrange(noised_input, "b v t c h w -> (b v t) c h w " )
        ...
        if self.loss_type == "l2":
            if self.use_additional_loss:
                predict_seq = rearrange(
                    predict, "(b v t) ... -> b v t ...", t=self.num_frames, v=self.num_cameras
                )  ## modify
                target_seq = rearrange(target, "(b v t) ... -> b v t ...", t=self.num_frames, v=self.num_cameras)
                bs = target.shape[0] // self.num_frames // self.num_cameras
                aux_loss = (
                    (target_seq[:, :, 1:] - target_seq[:, :, :-1]) - (predict_seq[:, :, 1:] - predict_seq[:, :, :-1])
                ) ** 2
                tmp_h, tmp_w = aux_loss.shape[-2], aux_loss.shape[-1]
                aux_loss = rearrange(aux_loss, "b v t c h w -> b (v t h w) c", c=4)
                aux_w = F.normalize(aux_loss, p=2)
                aux_w = rearrange(
                    aux_w, "b (v t h w) c -> b v t c h w", t=self.num_frames - 1, h=tmp_h, w=tmp_w, v=self.num_cameras
                )
                aux_w = 1 + torch.cat((torch.zeros(bs, self.num_cameras, 1, *aux_w.shape[3:]).to(aux_w), aux_w), dim=2)
                aux_w = rearrange(aux_w, "b v t ... -> (b v t) ...").reshape(target.shape[0], -1)
                ...
                return (
                    torch.mean((w * (predict - target) ** 2).reshape(target.shape[0], -1) * aux_w.detach(), 1).mean()
                    + self.additional_loss_weight * hf_loss
                )

            else:
                return torch.mean((w * (predict - target) ** 2).reshape(target.shape[0], -1), 1)
            ...
            if self.use_additional_loss:
                ...
                return (
                    torch.mean((w * (predict - target).abs()).reshape(target.shape[0], -1) * aux_w.detach(), 1).mean()
                    + self.additional_loss_weight * hf_loss
                )
            else:
                return torch.mean((w * (predict - target).abs()).reshape(target.shape[0], -1), 1)
        else:
