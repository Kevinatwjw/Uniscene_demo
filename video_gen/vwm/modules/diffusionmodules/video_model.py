...
from vwm.modules.attention import BasicMultiviewTransformerBlock, BasicTransformerBlock  # # modify
...
class VideoUNet(nn.Module):
    def __init__(
        ...
        if neighboring_view_pair is not None:  ## modify -->replace
            # MemoryEfficientCrossAttention = []
            for name, mod in list(self.named_modules()):
                if isinstance(mod, BasicTransformerBlock):  ### MemoryEfficientCrossAttention.
                    # MemoryEfficientCrossAttention.append(name)
                    _set_module(
                        self,
                        name,
            ...
            # print("******************", MemoryEfficientCrossAttention)
            # 'input_blocks.1.1.transformer_blocks.0', 'input_blocks.2.1.transformer_blocks.0', 'input_blocks.4.1.transformer_blocks.0',
            # 'input_blocks.5.1.transformer_blocks.0', 'input_blocks.7.1.transformer_blocks.0', 'input_blocks.8.1.transformer_blocks.0',
            # 'middle_block.1.transformer_blocks.0', 'output_blocks.3.1.transformer_blocks.0', 'output_blocks.4.1.transformer_blocks.0',
            # 'output_blocks.5.1.transformer_blocks.0', 'output_blocks.6.1.transformer_blocks.0', 'output_blocks.7.1.transformer_blocks.0',
            # 'output_blocks.8.1.transformer_blocks.0', 'output_blocks.9.1.transformer_blocks.0', 'output_blocks.10.1.transformer_blocks.0',
    ...
    def forward(
        ...
        hs = list()
        # print(timesteps, self.model_channels)
        try:
        ...
        if num_frames > 1 and context.shape[0] != x.shape[0]:
            ...
            context = repeat_as_img_seq(context, int(x.shape[0] / context.shape[0]))
        ...
        if self.num_classes is not None:
            if num_frames > 1 and y.shape[0] != x.shape[0]:
                ...
                y = repeat_as_img_seq(y, int(x.shape[0] / y.shape[0]))  ##modify
        ...
        h = self.middle_block(h, emb, context=context, time_context=time_context, num_frames=num_frames)
