...
class OpenAIWrapper(IdentityWrapper):
    def forward(
        ...
        # if "concat" in c and num_frames > 1 and c["concat"].shape[0] != x.shape[0]:
        # assert c["concat"].shape[0] == x.shape[0] // num_frames, f"{c['concat'].shape} {x.shape}"
        # c["concat"] = repeat_as_img_seq(c["concat"], 6)
        # x = torch.cat((x, c.get("concat", torch.Tensor(list()).type_as(x))), dim=1)  ## modify

        x = torch.cat((x, c["concat"]), dim=1)
        # print("x.shape,  concat_occ", x.shape, c["concat_occ"].shape)
        x = x + c["concat_occ"]

        # if "occ_semantic" in c and "occ_depth" in c:
        #     x = torch.cat((x, c["occ_semantic"], c["occ_depth"]  ), dim=1)
