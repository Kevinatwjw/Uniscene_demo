...
class GeneralConditioner(nn.Module):
    ...
    def forward(self, batch: Dict, force_zero_embeddings: Optional[List] = None) -> Dict:
        ...
        for embedder in self.embedders:
            if embedder.input_key in ["occ_semantic", "occ_depth"]:
                if embedder.legacy_ucg_val is not None:
                    batch = self.possibly_get_ucg_val(embedder, batch)
                if embedder.input_key in batch:
                    emb_out = embedder(batch[embedder.input_key])
                output_occ[embedder.input_key] = emb_out
            else:
                ...
                with embedding_context():
                    if hasattr(embedder, "input_key") and embedder.input_key is not None:
                        ...
                        if embedder.input_key in batch:
                            ...
                            embedder.input_key in ["command", "trajectory", "speed", "angle", "goal"]
                            and embedder.add_sequence_dim
                        ):  ## modify
                            emb_dim = embedder.num_features * embedder.outdim
                            emb_out = torch.zeros(
                                (batch["cond_aug"].shape[0], 1, emb_dim), device=batch["cond_aug"].device
                            )  ## modify
                        else:  # addition
                            continue

                        if emb_out.dim() == 5:
                            emb_out = rearrange(emb_out, "v t ... -> (v t) ...")
        ...
        output["concat_occ"] = output_occ["occ_semantic"] + output_occ["occ_depth"]
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def forward(self, image, no_dropout=False):
        ...
        if self.output_tokens:
            ...
            assert z.dim() == 3
            z_pad = torch.cat(
                (z, torch.zeros(z.shape[0], self.max_length - z.shape[1], z.shape[2], device=z.device)), 1
            )
...
class Controlnetencoder_semantic(AbstractEmbModel):
    def __init__(self, dims=2, in_dims=1, out_dims=8):
        super().__init__()
        self.encoder_0 = nn.Sequential(
            conv_nd(dims, in_dims, 32, 3, padding=1, stride=2),
            nn.SiLU(),
        ...
        self.encoder_1 = nn.Sequential(
            BasicResNetBlock(32, 32, zero_init=False),
        )
        self.encoder_2 = nn.Sequential(
            BasicResNetBlock(32, 32, zero_init=False),
            zero_module(conv_nd(dims, 32, out_dims, 3, padding=1, stride=1)),
        )
...
class Controlnetencoder_depth(AbstractEmbModel):
    def __init__(self, dims=2, in_dims=1, out_dims=8):
        super().__init__()
        self.encoder_0 = nn.Sequential(
            conv_nd(dims, in_dims, 32, 3, padding=1, stride=2),
