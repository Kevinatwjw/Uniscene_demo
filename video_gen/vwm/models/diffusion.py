...
class DiffusionEngine(LightningModule):
    ...
    def get_input(self, batch):
        ...
        if len(input_shape) != 4:  # is an image sequence
            if len(input_shape) == 6:  ## modify
                assert input_shape[2] == self.num_frames
                batch[self.input_key] = rearrange(batch[self.input_key], "b v t c h w -> (b v t) c h w")
            else:
    ...
    def get_batch_adj(self, keys, value_dict, N: Union[List, ListConfig], device="cuda", views=6):
        batch = dict()
        for key in keys:
            if key in value_dict:
                # print( key, value_dict[key].shape )
                if key in ["fps", "fps_id", "motion_bucket_id", "cond_aug"]:
                    # if value_dict[key].ndim >1:   value_dict[key]=value_dict[key][0]
                    value_dict[key] = value_dict[key].view(-1)[0].unsqueeze(0)
                    batch[key] = repeat(value_dict[key].to(device), "1 ... -> (b f) ...", b=N[0], f=views)
                elif key in ["command", "trajectory", "speed", "angle", "goal"]:
                    if value_dict[key].ndim > 1:
                        value_dict[key] = value_dict[key][0]
                    ...
                    batch[key] = rearrange(value_dict[key], "b v t c h w -> (b v t) c h w")
                else:
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        bs, view, frames, channels, hights, widths = batch["img_seq"].shape

        x = self.get_input(batch)
        x = self.encode_first_stage(x)
        batch["global_step"] = self.global_step

        batch = self.get_batch_adj(
            keys=list(set([x.input_key for x in self.conditioner.embedders])), value_dict=batch, N=[frames]
        )
        # for key, value in  batch.items():  print(f"{key} : {value.shape}")
    ...
    def configure_optimizers(self):
        ...
        if self.slow_spatial_layers:
            param_dicts = [
                ...
                {"params": [p for n, p in self.model.named_parameters() if "time_stack" not in n], "lr": lr * 0.1},
            ]
        elif self.train_peft_adapters:
            param_dicts = [{"params": [p for n, p in self.model.named_parameters() if "adapter" in n]}]
        else:
    ...
    def sample(
        ...
        **kwargs,
        ...
        if self.replace_cond_frames:
            ...
            cond_mask = rearrange(cond_mask, "(b v t) -> b v t", t=self.num_frames, v=self.num_cameras)  ## modify
            cond_mask[:, :, cond_indices] = 1
            cond_mask = rearrange(cond_mask, "b v t -> (b v t)")
    ...
    @torch.no_grad()
    def log_images(self, batch: Dict, N: int = 25, sample: bool = True, ucg_keys: List[str] = None, **kwargs) -> Dict:
        ...
        batch = self.get_batch_adj(
            keys=list(set([x.input_key for x in self.conditioner.embedders])), value_dict=batch, N=[N]
        )  ## modify
        # for key, value in  batch.items():  print(f"{key} : {value.shape}")

        c, uc = self.conditioner.get_unconditional_conditioning(
            batch, force_uc_zero_embeddings=ucg_keys if len(self.conditioner.embedders) > 0 else list()
        )

        sampling_kwargs = dict()
        N = min(x.shape[0], N)
        # N = x.shape[0]
        x_flatten = rearrange(x, "(b v t) c h w -> b v t c h w", v=self.num_cameras, t=N)  ## modify
        x_flatten = x_flatten.to(self.device)[:, :, :N]
        x = rearrange(x_flatten, "b v t c h w -> (b v t) c h w", v=self.num_cameras, t=N)
        ...
        if sample:
            with self.ema_scope("Plotting"):
                samples = self.sample(
                    c, cond_frame=z, shape=z.shape[1:], uc=uc, N=N * self.num_cameras, **sampling_kwargs
                )
        ...
        log["occ_depth"] = batch["occ_depth"]
        log["occ_semantic"] = batch["occ_semantic"]
