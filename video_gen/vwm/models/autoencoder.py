...
class AbstractAutoencoder(LightningModule):
    """This is the base class for all autoencoders, including image
    autoencoders, image autoencoders with discriminators, unCLIP models, etc.
    Hence, it is fairly general, and specific features (e.g. discriminator
    training, encoding, decoding) must be implemented in subclasses."""

    def __init__(self, ema_decay: Union[None, float] = None, monitor: Union[None, str] = None, input_key: str = "img"):
        super().__init__()
    ...
    def apply_ckpt(self, ckpt: Union[None, str, dict]):
        if ckpt is None:
            ...
            ckpt = {"target": "vwm.modules.checkpoint.CheckpointEngine", "params": {"ckpt_path": ckpt}}
...
class AutoencodingEngine(AbstractAutoencoder):
    """Base class for all image autoencoders that we train, like VQGAN or
    AutoencoderKL (we also restore them explicitly as special cases for legacy
    reasons).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.trainable_ae_params is not None:
            self.ae_optimizer_args = default(ae_optimizer_args, [dict() for _ in range(len(self.trainable_ae_params))])
        ...
        if self.trainable_disc_params is not None:
            self.disc_optimizer_args = default(
                disc_optimizer_args, [dict() for _ in range(len(self.trainable_disc_params))]
            )
    ...
    def encode(
        self, x: torch.Tensor, return_reg_log: bool = False, unregularized: bool = False
    ...
    def inner_training_step(self, batch: dict, batch_idx: int, optimizer_idx: int = 0) -> torch.Tensor:
        ...
        if optimizer_idx == 0:
            ...
            self.log_dict(log_dict_ae, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=False)
            self.log("loss", ae_loss.mean().detach(), prog_bar=True, logger=False, on_epoch=False, on_step=True)
            return ae_loss
    ...
    def _validation_step(self, batch: dict, batch_idx: int, postfix: str = "") -> Dict:
        ...
        self.log(f"val{postfix}/loss/rec", log_dict_ae[f"val{postfix}/loss/rec"], sync_dist=True)
    ...
    def configure_optimizers(self) -> List[torch.optim.Optimizer]:
        ...
        opt_ae = self.instantiate_optimizer_from_config(
            ae_params, default(self.lr_g_factor, 1.0) * self.learning_rate, self.optimizer_config
        )
        opts = [opt_ae]
...
class AutoencodingEngineLegacy(AutoencodingEngine):
    def __init__(self, embed_dim: int, **kwargs):
        ...
        super().__init__(
            encoder_config={"target": "vwm.modules.diffusionmodules.model.Encoder", "params": ddconfig},
            decoder_config={"target": "vwm.modules.diffusionmodules.model.Decoder", "params": ddconfig},
            **kwargs,
        )
        self.quant_conv = nn.Conv2d(
            (1 + ddconfig["double_z"]) * ddconfig["z_channels"], (1 + ddconfig["double_z"]) * embed_dim, 1
        )
...
class AutoencoderKL(AutoencodingEngineLegacy):
    def __init__(self, **kwargs):
        super().__init__(
            regularizer_config={"target": ("vwm.modules.autoencoding.regularizers.DiagonalGaussianRegularizer")},
            **kwargs,
        )
