...
from .subsets import NuScenesDataset, NuScenesDatasetMVTOSOCC, YouTubeDataset
...
def dataset_mapping(
    subset_list: list, target_height: int, target_width: int, num_frames: int, num_cameras: int = 0
):  ## modify
    datasets = list()
    for subset_name in subset_list:
        ...
        elif subset_name == "NuScenesDatasetMV":
            datasets.append(
                NuScenesDatasetMV(
        ...
        elif subset_name == "NuScenesDatasetMVTOS":
            datasets.append(
                NuScenesDatasetMVTOS(
                    target_height=target_height,
                    target_width=target_width,
                    num_frames=num_frames,
                    num_cameras=num_cameras,
        ...
        elif subset_name == "NuScenesDatasetMVTOSOCC":
            datasets.append(
                NuScenesDatasetMVTOSOCC(
                    target_height=target_height,
                    target_width=target_width,
                    num_frames=num_frames,
                    num_cameras=num_cameras,
...
class MultiSourceSamplerDataset(Dataset):
    def __init__(
        self, subsets, probs, samples_per_epoch=1000, target_height=320, target_width=576, num_frames=25, num_cameras=6
    ):
        self.subsets = dataset_mapping(subsets, target_height, target_width, num_frames, num_cameras)
