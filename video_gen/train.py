...
torch.set_float32_matmul_precision("medium")  ## modify
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "-r", "--resume", type=str, const=True, default="", nargs="?", help="resume from logdir or checkpoint in logdir"
    )
    parser.add_argument(
        "-b",
        "--base",
    ...
    parser.add_argument("--no_test", type=str2bool, const=True, default=True, nargs="?", help="disable test")
    parser.add_argument("-p", "--project", help="name of new or path to existing project")
    parser.add_argument(
        "-d", "--debug", type=str2bool, nargs="?", const=True, default=False, help="enable post-mortem debugging"
    )
    ...
    parser.add_argument("--no_base_name", type=str2bool, nargs="?", const=True, default=False, help="no config name")
    if version.parse(pl.__version__) >= version.parse("2.0.0"):
        parser.add_argument(
            "--resume_from_checkpoint", type=str, default=None, help="single checkpoint file to resume from"
        )
    parser.add_argument("--n_devices", type=int, default=8, help="number of gpus in training")
    parser.add_argument(
        "--finetune", type=str, default="ckpts/pytorch_model.bin", help="path to checkpoint to finetune from"
    )
...
class SetupCallback(Callback):
    def __init__(self, resume, now, logdir, ckptdir, cfgdir, config, lightning_config, debug, ckpt_name=None):
        super().__init__()
        self.resume = resume
        self.now = now
        self.logdir = logdir
...
class ImageLogger(Callback):
    def __init__(
        ...
        self.num_cameras = num_cameras  ## modify
    ...
    def log_local(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        ...
        for log_type in images:
            if isheatmap(images[log_type]):
                fig, ax = plt.subplots()
                ax = ax.matshow(images[log_type].cpu().numpy(), cmap="hot", interpolation="lanczos")
                plt.colorbar(ax)
                plt.axis("off")
                ...
                img_seq = rearrange(
                    img_seq, "(b n t) c h w -> b n t h w c", n=self.num_cameras, t=self.num_frames
                )  ## modify
                if img_seq.dim() == 6:
                    B, cam_view, T = img_seq.shape[:3]
                    for b_i in range(B):
                        cur_img_seq_mv = img_seq[b_i].numpy()  # [v, t h w c]
                        cur_img_seq_mv = (cur_img_seq_mv * 255).astype(np.uint8)  # [v, t h w c]
                        filename = "{}_epoch{:02}_batch{:04}_step{:06}.mp4".format(
                            log_type, current_epoch, batch_idx, global_step
                        )
                        ori_img_shape = list(cur_img_seq_mv.shape[2:])
                        img_shape = [T, ori_img_shape[0] * 2, ori_img_shape[1] * 3, 3]
                        cur_img_seq = np.zeros(img_shape).astype(np.uint8)

                        for view_idx in range(cam_view):
                            current_view = cur_img_seq_mv[view_idx]
                            if view_idx < 3:
                                cur_img_seq[
                                    :,
                                    : ori_img_shape[0],
                                ...
                                cur_img_seq[
                                    :,
                                    ori_img_shape[0] :,
                                    (view_idx - 3) * ori_img_shape[1] : ((view_idx - 3) * ori_img_shape[1])
                                    + ori_img_shape[1],
                                ] = current_view
            ...
            elif "occ_depth" in log_type:
                grid = torchvision.utils.make_grid(images[log_type], nrow=self.num_frames)
                if self.rescale:
                    grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
                grid = grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
                grid = grid.numpy()
                grid = (grid * 255).astype(np.uint8)
                filename = "{}_epoch{:02}_batch{:04}_step{:06}_occ_depth.png".format(
                    log_type, current_epoch, batch_idx, global_step
                )
                dir_path = os.path.join(root, log_type)
                os.makedirs(dir_path, exist_ok=True)
                ...
                grid = grid.numpy()
                grid = (grid * 255).astype(np.uint8)
                filename = "{}_epoch{:02}_batch{:04}_step{:06}_occ_semantic.png".format(
                    log_type, current_epoch, batch_idx, global_step
                )
                dir_path = os.path.join(root, log_type)
                os.makedirs(dir_path, exist_ok=True)
    ...
    try:
        ...
        default_logger_cfgs = {
            "csv": {
                ...
                "params": {"name": "testtube", "save_dir": logdir},  # hack for sbord fanatics
            }
        }
        ...
        default_callbacks_cfg = {
            ...
            "image_logger": {"target": "train.ImageLogger", "params": {"batch_frequency": 1000, "clamp": True}},
        ...
        if "batch_size" in config.data.params:
            ...
            bs, base_lr = (config.data.params.train.loader.batch_size, config.model.base_learning_rate)
        if cpu:
            ngpu = 1
        else:
