...
# @MODELS.register_module()
# class Decoder2D_withT(BaseModule):
#     def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
#                  attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
#                  resolution, z_channels, give_pre_end=False,t_shape=5,**ignorekwargs):
#         super().__init__()
#         self.ch = ch
#         self.temb_ch = 0
#         self.num_resolutions = len(ch_mult)
...
#                                        dropout=dropout)
#         self.mid.attn_1 = nn.Sequential(AttnBlock(block_in),
#                                         AttnBlock3D(block_in,t_shape=self.t_shape))
...
#                 if curr_res in attn_resolutions:
#                     print('[*] Dec has Attn at i_level, i_block: %d, %d' % (i_level, i_block))
#                     attn.append( nn.Sequential(AttnBlock(block_in),AttnBlock3D(block_in,t_shape=self.t_shape)) )
#             up = nn.Module()
#             up.block = block
#             up.attn = attn
#             if i_level != 0:
