...
class MultiLoss(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for loss_func in self.losses:
            ...
            tot_loss += loss
            loss_dict.update({loss_func.__class__.__name__: loss.detach().item() / loss_func.weight})
            if writer and self.iter_counter % 10 == 0:
                writer.add_scalar(f"loss/{loss_func.__class__.__name__}", loss.detach().item(), self.iter_counter)
        if writer and self.iter_counter % 10 == 0:
