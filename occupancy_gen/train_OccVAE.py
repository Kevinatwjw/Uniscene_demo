...
def main(local_rank, args):
    ...
    from utils.metric_util import multi_step_MeanIou
    ...
    CalMeanIou_sem = multi_step_MeanIou(
        unique_label, cfg.get("ignore_label", -100), unique_label_str, "sem", times=cfg.get("return_len_", 10)
    )
    CalMeanIou_vox = multi_step_MeanIou(
        [1], cfg.get("ignore_label", -100), ["occupied"], "vox", times=cfg.get("return_len_", 10)
    )
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (input_occs, target_occs, metas) in enumerate(train_dataset_loader):
            ...
            if i_iter % print_freq == 0 and local_rank == 0:
                ...
                logger.info(
                    "[TRAIN] Epoch %d Iter %5d/%d: Loss: %.3f (%.3f), grad_norm: %.3f, lr: %.7f, time: %.3f (%.3f)"
                    % (
                        epoch,
                        i_iter,
